...
class FolderEventsListenerFactory {
    ...
    func applicationTestLogsEventsListener(at derivedDataFolder: DerivedDataFolder) -> FolderEventsListener {
        ...
        if derivedDataFolder.type == .appcode {
             ...
             */
            pathRegex = "/Logs/Test/Test-.+-[0-9]{4}.[0-9]{2}.[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}-[+-][0-9]{3,4}.xcresult/Data"
}
        return NonRecursiveFolderEventsListener(folderPath: derivedDataFolder.path, filter: FolderEventFilter.known & FolderEventFilter.type(.folder) & FolderEventFilter.pathRegex(pathRegex))
    }
}
